...
from world_scanner import WorldScanner, WorldPosition, ScanLogHandler, ScanWorker
import numpy as np
...
class OverlayController(QMainWindow):
    ...
    def create_overlay_controls(self, parent_layout: QVBoxLayout, settings: Dict[str, Any]) -> None:
        ...
        # Toggle button
        self.toggle_btn = QPushButton("Toggle Overlay (F10)")
    ...
    def connect_settings_handlers(self) -> None:
        ...
        self.confidence_slider.valueChanged.connect(self.confidence_input.setValue)
        self.confidence_input.valueChanged.connect(self.confidence_slider.setValue)
        
        # Connect scale controls
        def on_scale_slider_change(value: int) -> None:
            self.scale_input.setValue(value / 10.0)
            self.save_settings()
        
        def on_scale_spinbox_change(value: float) -> None:
            self.scale_slider.setValue(int(value * 10))
            self.save_settings()
        
        self.scale_slider.valueChanged.connect(on_scale_slider_change)
        self.scale_input.valueChanged.connect(on_scale_spinbox_change)
        
        # Connect to save settings
        self.thickness_slider.valueChanged.connect(self.save_settings)
        self.font_size_slider.valueChanged.connect(self.save_settings)
        self.text_thickness_slider.valueChanged.connect(self.save_settings)
        self.cross_thickness_slider.valueChanged.connect(self.save_settings)
