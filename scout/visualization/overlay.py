...
class Overlay(QWidget):
    ...
    def __init__(self, window_manager: WindowManager, 
        ...
        # Initialize OCR region tracking
        self.ocr_region: Optional[Dict[str, int]] = None
    ...
    def set_debug_mode(self, enabled: bool) -> None:
        ...
        """
        logger.info(f"Setting overlay debug mode to: {enabled}")
        self.debug_enabled = enabled
        logger.debug(f"Debug mode set to: {enabled}, OCR region present: {self.ocr_region is not None}")
        
    def set_ocr_region(self, region: Optional[Dict[str, int]]) -> None:
        """
        Set the OCR region to be displayed in debug mode.
        
        Args:
            region: Dictionary containing region coordinates (left, top, width, height)
                   or None to clear the region
        """
        logger.info(f"Setting overlay OCR region: {region}")
        self.ocr_region = region
        logger.debug(f"OCR region set, debug mode is: {self.debug_enabled}")

    def update(self, matches: List[GroupedMatch]) -> None:
        """Update the overlay window with pattern matching results and debug information."""
        ...
        x, y, width, height = pos
        logger.debug(f"Window base position: ({x}, {y}), size: {width}x{height}")
        ...
        try:
            ...
            # Draw OCR region if debug mode is enabled
            if self.debug_enabled and self.ocr_region:
                logger.debug(f"Drawing OCR region in debug mode: {self.ocr_region}")
                # Adjust region position by client offset
                ocr_x = self.ocr_region['left'] - client_offset_x
                ocr_y = self.ocr_region['top'] - client_offset_y
                ocr_width = self.ocr_region['width']
                ocr_height = self.ocr_region['height']
                
                # Log all coordinate transformations
                logger.debug(f"OCR region original: left={self.ocr_region['left']}, top={self.ocr_region['top']}")
                logger.debug(f"Client offset: x={client_offset_x}, y={client_offset_y}")
                logger.debug(f"Window position: x={x}, y={y}")
                logger.debug(f"Final OCR position: x={ocr_x}, y={ocr_y}")
                
                # Draw pink rectangle for OCR region
                cv2.rectangle(
                    overlay,
                    (ocr_x, ocr_y),
                    (ocr_x + ocr_width, ocr_y + ocr_height),
                    (255, 192, 203),  # Pink color in BGR
                    2  # Thickness
                )
                
                # Draw "OCR Region" label
                cv2.putText(
                    overlay,
                    "OCR Region",
                    (ocr_x, ocr_y - 5),
                ...
                logger.debug("OCR region drawn on overlay")
            else:
                logger.debug(f"Skipping OCR region drawing - debug_enabled: {self.debug_enabled}, ocr_region present: {self.ocr_region is not None}")
